"""
This code is taken from the Github repository of the AIMA book (https://github.com/aimacode/aima-python)
The important parts here are best_first_graph_search (of which A* is a particular case where f = g + h),
which has been slightly modified w.r.t. the original version
...
def best_first_graph_search(problem, f, n_saved_explored_states=0, max_iter=float('inf')):
    ...
    # this Python priority queue is super slow!
    ...
    while frontier and nexplored < max_iter:
        node = frontier.pop()
        if problem.goal_test(node.state):
            return node, nexplored, explored_states
        explored.add(node.state)
        if nexplored < n_saved_explored_states:
            explored_states.append(node.state)
        nexplored += 1
        if nexplored % 100 == 0:
            print("Explored nodes: " + str(nexplored))
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
                frontier.append(child)
    ...
    return None, nexplored, explored_states
...
def astar_search(problem, n_saved_explored_states=0, h=None, max_iter=float('inf')):
    """A* search is best-first graph search with f(n) = g(n)+h(n).
    You need to specify the h function when you call astar_search, or
    else in your Problem subclass."""
    h = memoize(h or problem.h, 'h')
    return best_first_graph_search(problem, lambda n: n.path_cost + h(n),
                                   n_saved_explored_states=n_saved_explored_states,
                                   max_iter=max_iter)
